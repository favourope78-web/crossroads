...
World Expansion & Location System delivered (2026-09-04): scalable, data-driven locations — `Gameplay/Locations/` (`LocationManager` pure-derivation runtime: unlock rules reuse the gate language with OR-across/AND-within semantics; connection-graph travel + returns; first-arrival world changes applied once via persisted `loc_visited_` flags; hub locations open-by-design) + `LocationServices` (boot + autosave + MapSnapshot) + `LocationEvents` (arrival carries checkpoint + env profile) + `MapHUD` (current / travel-to / locked with requirement hints) + `LocationTransitionFader` (Android-suitable: 0.2s unscaled fade → anchor teleport → env apply; no scene loads on the hot path). Location ids share the area-id namespace so AreaTriggers and the travel graph never desync; **no save-schema change** (unlocks/current/visits/world state already v5). Three polished prototype locations in the regenerated scene (145→162 roots): Fracture Hall (hub/story), North Annex (combat — Warden relocated, ability-gated via any route, hidden ember cache), Tidewell Shrine (NEW east zone, tide-decision-gated, Sera relocates there). Location kits under `Assets/Game/Locations/<Location>/`; new LocationTests [64]–[71]: **962/962** headless checks incl. the full task-14 flow (hall→decision→annex→encounter→objective→world change→return→hall reflects it→restart restored); validation + both compile configs green; see `WORLD_EXPANSION_REPORT.md`.
