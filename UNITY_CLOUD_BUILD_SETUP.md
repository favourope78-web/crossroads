...
| `AndroidDevBuild.cs(51): CS0117 'PlayerSettings' does not contain a definition for 'activeInputHandling'` / `CS0103 'ActiveInputHandling'` | Build #2. That API never existed; Active Input Handling is only the serialized `activeInputHandler` field of `ProjectSettings.asset` (0 old / 1 new / 2 both). Fixed in `52c1f66`: `Configure()` sets it through `SerializedObject` (what the Player Settings inspector and the Input System package do). The value is also committed as `activeInputHandler: 2` in the asset |
| `warning CS0618: 'PlayerSettings.SetXxx(BuildTargetGroup, …)' is obsolete: Use … NamedBuildTarget` | Warnings only, never fatal. Both editor scripts now use `NamedBuildTarget.Android` (`52c1f66`) |
